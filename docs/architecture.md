...
- The current M2 renderer scaffold treats the frame arena as a `640x480` xRGB8888 internal framebuffer. `ENGINE64` provides a deterministic neon title-scene palette, and the boot runtime presents that internal frame to GOP through direct or red/blue-swapped conversion for the common BGR, RGB, and matching bitmask layouts.
