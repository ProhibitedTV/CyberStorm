...
- The current M2 renderer scaffold treats the frame arena as a `640x480` xRGB8888 internal framebuffer. `ENGINE64` provides a deterministic expanded neon megacity title-scene palette, and the boot runtime presents that internal frame to GOP through direct or red/blue-swapped conversion for the common BGR, RGB, and matching bitmask layouts.
- `scripts/deploy-x64-vm.ps1` is the supported local demo path for the forward release. It creates a UEFI VirtualBox profile, attaches `cyberstorm-x64.iso`, optionally captures the title frame, and can run a small menu input smoke without touching the legacy BIOS VM.
