...
function Write-GeneratedDemoInclude {
    ...
    for ($demoIndex = 0; $demoIndex -lt $demos.Count; $demoIndex++) {
        ...
        if ($captureRole -notin $captureRoles) {
            throw ("Demo '{0}' in {1} must use one of the supported CaptureRole values: {2}." -f $name, $SourcePath, ($captureRoles -join ', '))
        }
...
$render3DOverride = $DebugRender3D.IsPresent
if ($render2DOverride -and $render3DOverride) {
    throw 'Use either -DebugRender2D or -DebugRender3D, not both.'
}
$debugProfile = $DebugBuild.IsPresent -or $seedProvided -or $DebugOverlay.IsPresent -or $DebugStartInGame.IsPresent -or $startSectorProvided -or $DebugDemoBoot.IsPresent -or $demoIndexProvided -or $DebugFrontendVerify.IsPresent -or $frontendScenarioProvided -or $DebugRuntimeVerify.IsPresent -or $verifyCorruptDemoProvided -or $verifyCorruptFrontendProvided -or $render2DOverride -or $render3DOverride
...
$audioModeValue = if ($ExperimentalMusic.IsPresent) { 1 } else { 0 }
$audioModeName = if ($ExperimentalMusic.IsPresent) { 'EXPERIMENTAL_MUSIC' } else { 'SFX_ONLY' }
$audioConfigLines = @(
    '; generated by scripts/build.ps1'
    'AUDIO_MODE_SFX_ONLY EQU 0'
    'AUDIO_MODE_EXPERIMENTAL_MUSIC EQU 1'
    ("AUDIO_MODE EQU {0}" -f $audioModeValue)
    ("AUDIO_MUSIC_ENABLED EQU {0}" -f ([int]$ExperimentalMusic.IsPresent))
)
...
$audioSummaryLines = @(
    ...
    ("Looping music: {0}" -f ($(if ($ExperimentalMusic.IsPresent) { 'enabled (experimental)' } else { 'disabled in release' })))
    'Backend order: SB16 when available, PC speaker fallback otherwise.'
    'Policy: one-shot SFX are the supported release baseline; looping music is opt-in while audio v2 is still experimental.'
)
$renderSummaryLines = @(
    ("Scene renderer: {0}" -f $sceneRenderModeName)
    ("Gameplay renderer: {0}" -f $gameplayRenderModeName)
    'Debug switches: -DebugRender2D keeps the legacy 2D oracle, -DebugRender3D forces the low-poly 3D room path explicitly.'
)
...
Write-BuildReport `
    ...
    -Warnings $warnings `
    -ArtifactPaths @($generatedArtPath, $generatedPresentationPath, $generatedGeometryPath, $generatedSectorContentPath, $generatedMapsPath, $generatedDemosPath, $generatedRuntimeVerifyPath, $generatedMusicPath, $replayReportPath, $balanceReportPath, $regressionReportPath, $frontendVerifyReportPath, $vmSmokeReportPath, $runtimeVerifyReportPath, $showcaseReportPath, $generatedBankLayoutPath, $mapBankBinPath, $presentationBankBinPath, $geometryBankBinPath, $bootBinPath, $stage2BinPath, $bootList, $gameList, $bootConfig, $debugConfig, $audioConfig, $imgPath, $vfdPath) + $readmeScreenshotArtifacts + @($reportPath, $screenshotSync.RotationStatePath) + $frontendVerifyArtifacts + $vmSmokeArtifacts + $runtimeVerifyArtifacts + $showcaseArtifacts `
    -Layout $layout
