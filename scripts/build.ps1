...
$sceneRenderModeName = if ($render2DOverride) { 'SCENES_2D_ORACLE' } else { 'SCENES_3D' }
$gameplayRenderModeValue = if ($render2DOverride) { 0 } else { 1 }
$gameplayRenderModeName = if ($render2DOverride) { 'GAMEPLAY_2D_ORACLE' } else { 'GAMEPLAY_3D_ADVENTURE' }
$debugConfigLines = @(
...
$renderSummaryLines = @(
    ...
    'Debug switches: -DebugRender2D keeps the current adventure slice but draws it through the flat 2D oracle; -DebugRender3D forces the low-poly 3D path explicitly.'
)
