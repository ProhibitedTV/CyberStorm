param(
    ...
    [switch]$DebugRenderSentinels,
    [ValidateRange(0, 5)]
    [Nullable[int]]$DebugRenderStage,
    [ValidateRange(0, 4)]
    [Nullable[int]]$DebugRenderRoomStage
)
    ...
    for ($y = 0; $y -lt $height; $y++) {
        ...
        for ($x = 0; $x -lt $width; $x++) {
            ...
            if ($tile -eq 0) {
                ...
                $ring = (($localX - 15) * ($localX - 15) + ($localY - 15) * ($localY - 15))
                $scan = (($localY % 6) -eq 0)
                if ($ring -lt 52) {
                    $color = Join-Xrgb 255 230 109
                } elseif ($ring -lt 104) {
                    $color = Join-Xrgb 255 64 128
                } elseif ($scan) {
                    $color = Join-Xrgb 80 24 64
                } else {
...
function New-X64BreachMeshChunk {
    ...
    & $addBox -132 -30 286 -70 68 366 10
    & $addBox 70 -30 286 132 68 366 10
    & $addBox -164 18 306 -132 58 358 1
    & $addBox 132 18 306 164 58 358 1
    & $addQuad @(-112, 8, 278) @(-88, 8, 278) @(-88, 34, 278) @(-112, 34, 278) 10
...
function Write-GeneratedGeometryInclude {
    ...
    $kitFloorBaseTexture = New-Object 'System.Collections.Generic.List[string]'
    ...
    $kitFloorTrimTexture = New-Object 'System.Collections.Generic.List[string]'
    ...
    $kitWallBaseTexture = New-Object 'System.Collections.Generic.List[string]'
    ...
    $kitWallTrimTexture = New-Object 'System.Collections.Generic.List[string]'
    ...
    $kitWallCapTexture = New-Object 'System.Collections.Generic.List[string]'
    ...
    $appendGeometry = {
        ...
        foreach ($faceIndex in 0..($Faces.Count - 1)) {
            ...
            $materialEntry = $materialMap[$materialKey]
            ...
            $faceTextureKeyOverride = if ($face.ContainsKey('TextureKey')) { ([string]$face['TextureKey']).Trim().ToLowerInvariant() } else { '' }
            $faceTextureKey = if ([string]::IsNullOrWhiteSpace($faceTextureKeyOverride)) { [string]$materialEntry.TextureKey } else { $faceTextureKeyOverride }
            if ($faceTextureKey -and $faceTextureKey -notmatch '^[a-z0-9]+(?:-[a-z0-9]+)*$') {
                throw ("{0} '{1}' face {2} in {3} must use lowercase TextureKey slugs. Found '{4}'." -f $OwnerKind, $OwnerKey, $faceIndex, $SourcePath, $face['TextureKey'])
            }

            $faceShadeModeOverride = if ($face.ContainsKey('ShadeMode')) { ([string]$face['ShadeMode']).Trim().ToLowerInvariant() } else { '' }
            $faceShadeMode = if ([string]::IsNullOrWhiteSpace($faceShadeModeOverride)) { [string]$materialEntry.ShadeMode } else { $faceShadeModeOverride }
            ...
            for ($triIndex = 1; $triIndex -lt ($indices.Count - 1); $triIndex++) {
                ...
                $payload.Add([byte]$materialEntry.Base)
                $payload.Add([byte]$materialEntry.Dither)
                $payload.Add([byte]$faceFx)
                $payload.Add([byte]$faceTextureIndex)
                $triangleCount += 1
            }
        }
    ...
    Add-AsmDataLines -Lines $lines -Label 'game3d_kit_floor_base_texture_table' -Directive 'db' -Values $kitFloorBaseTexture.ToArray() -ValuesPerLine 8
    ...
    Add-AsmDataLines -Lines $lines -Label 'game3d_kit_floor_trim_texture_table' -Directive 'db' -Values $kitFloorTrimTexture.ToArray() -ValuesPerLine 8
    ...
    Add-AsmDataLines -Lines $lines -Label 'game3d_kit_wall_base_texture_table' -Directive 'db' -Values $kitWallBaseTexture.ToArray() -ValuesPerLine 8
    ...
    Add-AsmDataLines -Lines $lines -Label 'game3d_kit_wall_trim_texture_table' -Directive 'db' -Values $kitWallTrimTexture.ToArray() -ValuesPerLine 8
    ...
    Add-AsmDataLines -Lines $lines -Label 'game3d_kit_wall_cap_texture_table' -Directive 'db' -Values $kitWallCapTexture.ToArray() -ValuesPerLine 8
...
$renderRoomStageProvided = $null -ne $DebugRenderRoomStage
...
if ($renderRoomStageProvided -and $render2DOverride) {
    throw '-DebugRenderRoomStage only applies to the 3D gameplay renderer.'
}
$debugProfile = $DebugBuild.IsPresent -or $seedProvided -or $DebugOverlay.IsPresent -or $DebugStartInGame.IsPresent -or $startSectorProvided -or $DebugDemoBoot.IsPresent -or $demoIndexProvided -or $DebugFrontendVerify.IsPresent -or $frontendScenarioProvided -or $DebugRuntimeVerify.IsPresent -or $verifyCorruptDemoProvided -or $verifyCorruptFrontendProvided -or $render2DOverride -or $renderReferenceOverride -or $renderMachineOverride -or $renderStageProvided -or $renderRoomStageProvided -or $DebugRenderSentinels.IsPresent
...
$debugRenderRoomStageValue = if ($renderRoomStageProvided) { [int]$DebugRenderRoomStage } else { 4 }
