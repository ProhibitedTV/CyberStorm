...
$debugProfile = $DebugBuild.IsPresent -or $seedProvided -or $DebugOverlay.IsPresent -or $DebugStartInGame.IsPresent -or $startSectorProvided -or $DebugDemoBoot.IsPresent -or $demoIndexProvided -or $DebugRuntimeVerify.IsPresent -or $verifyCorruptDemoProvided
...
Write-BuildReport `
    ...
    -Warnings $warnings `
    -ArtifactPaths @($generatedArtPath, $generatedPresentationPath, $generatedSectorContentPath, $generatedMapsPath, $generatedDemosPath, $generatedRuntimeVerifyPath, $generatedMusicPath, $replayReportPath, $balanceReportPath, $regressionReportPath, $vmSmokeReportPath, $runtimeVerifyReportPath, $showcaseReportPath, $generatedBankLayoutPath, $mapBankBinPath, $presentationBankBinPath, $bootBinPath, $stage2BinPath, $bootList, $gameList, $bootConfig, $debugConfig, $audioConfig, $imgPath, $vfdPath) + $readmeScreenshotArtifacts + @($reportPath, $screenshotSync.RotationStatePath) + $vmSmokeArtifacts + $runtimeVerifyArtifacts + $showcaseArtifacts `
    -Layout $layout
