...
# Deterministic pressure model. This mirrors the intended first runtime patch:
...
Add-Check 'Runtime integrity patch landed' $runtimeHasPressure "pressure-runtime=$runtimeHasPressure"

$failed = @($checks | Where-Object { -not $_.Passed })
$runtimePending = @($checks | Where-Object { $_.Name -eq 'Runtime integrity patch landed' -and -not $_.Passed })
$hardFailed = @($failed | Where-Object { $_.Name -ne 'Runtime integrity patch landed' })

$lines = New-Object System.Collections.Generic.List[string]
$lines.Add('CyberStorm x64 Integrity Pressure Harness')
$lines.Add(('Generated: {0:u}' -f (Get-Date).ToUniversalTime()))
$lines.Add('')
foreach ($check in $checks) {
    $status = if ($check.Passed) { 'PASS' } elseif ($check.Name -eq 'Runtime integrity patch landed') { 'PENDING' } else { 'FAIL' }
