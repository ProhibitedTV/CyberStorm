...
sector_num   db 1
current_district db 1
checkpoint_district db 1
...
; checkpoint_loadout stores [shield_count, pulse_count] for district restarts.
checkpoint_loadout db START_SHIELDS, START_PULSES
data_count   db 0
kill_count   db 0
checkpoint_kill_count db 0
score_total  dw 0
checkpoint_score_total dw 0
sector_score dw 0
...
menu_idle_ticks db 0
title_menu_index db TITLE_MENU_NEW_GAME
...
sector_score_table dw CAMPAIGN_DISTRICT_COUNT dup (0)
...
title_option_idle_demo_text db 'IDLE DEMO', 0
...
; The campaign generator emits per-district authored content. The runtime loads
; one district into these active buffers so gameplay/render code can keep using
...
adventure_realm_portal_y db 0
adventure_realm_required_gems db 0
adventure_realm_gem_count db 0
adventure_realm_switch_count db 0
adventure_realm_key_count db 0
adventure_realm_hazard_count db 0
adventure_realm_enemy_count db 0
adventure_realm_prop_count db 0
adventure_realm_chunk_count db 0
adventure_realm_gem_table db CAMPAIGN_MAX_GEM_BYTES dup (0)
adventure_realm_switch_table db CAMPAIGN_MAX_SWITCH_BYTES dup (0)
adventure_realm_key_table db CAMPAIGN_MAX_KEY_BYTES dup (0)
adventure_realm_hazard_table db CAMPAIGN_MAX_HAZARD_BYTES dup (0)
adventure_realm_enemy_table db CAMPAIGN_MAX_ENEMY_BYTES dup (0)
adventure_realm_chunk_min_x_table db CAMPAIGN_MAX_CHUNK_COUNT dup (0)
adventure_realm_chunk_max_x_table db CAMPAIGN_MAX_CHUNK_COUNT dup (0)
adventure_realm_chunk_min_y_table db CAMPAIGN_MAX_CHUNK_COUNT dup (0)
adventure_realm_chunk_max_y_table db CAMPAIGN_MAX_CHUNK_COUNT dup (0)
adventure_realm_chunk_base_height_table dw CAMPAIGN_MAX_CHUNK_COUNT dup (0)
adventure_realm_chunk_shelf_height_table dw CAMPAIGN_MAX_CHUNK_COUNT dup (0)
adventure_realm_chunk_ramp_dir_table db CAMPAIGN_MAX_CHUNK_COUNT dup (0)
adventure_realm_chunk_cliff_side_table db CAMPAIGN_MAX_CHUNK_COUNT dup (0)
adventure_realm_chunk_bridge_span_table db CAMPAIGN_MAX_CHUNK_COUNT dup (0)
adventure_realm_chunk_landmark_x_table db CAMPAIGN_MAX_CHUNK_COUNT dup (0)
adventure_realm_chunk_landmark_y_table db CAMPAIGN_MAX_CHUNK_COUNT dup (0)
adventure_realm_chunk_prop_budget_table db CAMPAIGN_MAX_CHUNK_COUNT dup (0)
adventure_realm_prop_x_table db CAMPAIGN_MAX_PROP_COUNT dup (0)
adventure_realm_prop_y_table db CAMPAIGN_MAX_PROP_COUNT dup (0)
adventure_realm_prop_mesh_table db CAMPAIGN_MAX_PROP_COUNT dup (0)
...
frontend_verify_line_1 db 'FRONTEND INPUT REACHED THE EXPECTED STATE.', 0
frontend_verify_line_2 db 'THE FRONTEND FLOW DIVERGED FROM EXPECTATION.', 0
frontend_verify_splash_name db 'SPLASH TO TITLE', 0
frontend_verify_title_start_name db 'MENU TO START', 0
frontend_verify_title_attract_name db 'MENU TO ATTRACT', 0
frontend_verify_splash_detail db 'EXPECT MENU ENTRY AFTER INPUT.', 0
frontend_verify_title_start_detail db 'EXPECT LIVE RUN ENTRY FROM MENU CONFIRM.', 0
frontend_verify_title_attract_detail db 'EXPECT ATTRACT HANDOFF AFTER MENU IDLE.', 0
